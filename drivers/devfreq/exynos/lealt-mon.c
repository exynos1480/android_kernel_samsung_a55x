 ...
 *				@cpu_grp's update_ms.
 * @hw:				The lealt_hwmon struct corresponding to this
...
DEFINE_STATIC_SRCU(lealt_srcu);

#define MAX_COUNT_LIM 0xFFFFFFFFFFFFFFFF

static unsigned int to_mon_idx_safe(unsigned int cpu,
				    const struct cpumask *mon_cpus)
{
	if (cpu > cpumask_first(mon_cpus))
		return cpu - cpumask_first(mon_cpus);
	else
		return 0;
}
...
static void update_counts_idle_core(struct memlat_cpu_grp *cpu_grp, int cpu)
{
	...
	for (i = 0; i < cpu_grp->num_mons; i++) {
		...
		mon_idx = to_mon_idx_safe(cpu, &mon->cpus);
				...
				int *state, struct cpuidle_device *dev)
{
	...
	srcu_idx = srcu_read_lock(&lealt_srcu);
	list_for_each_entry_srcu(cpu_grp, &cpu_grp_list, node,
				 srcu_read_lock_held(&lealt_srcu)) {
		if (!smp_load_acquire(&cpu_grp->initialized))
			continue;
	...
	srcu_read_unlock(&lealt_srcu, srcu_idx);
...
int get_cpumask(struct lealt_hwmon *hw, struct cpumask *cpus)
{
	...
	cpumask_copy(cpus, &mon->cpus);
	return 0;
}

static unsigned long get_cnt(struct lealt_hwmon *hw)
{
	struct memlat_mon *mon = to_mon(hw);
	...
	for_each_cpu(cpu, &mon->cpus) {
		struct cpu_data *cpu_data = to_cpu_data(cpu_grp, cpu);
		struct event_data *common_evs = cpu_data->common_evs;
		unsigned int mon_idx = to_mon_idx_safe(cpu, &mon->cpus);
			   ...
			   struct perf_event_attr *attr)
{
	...
	smp_store_release(&cpu_grp->initialized, true);
...
static int start_hwmon(struct lealt_hwmon *hw)
{
	int ret = 0;
	unsigned int cpu;
	struct memlat_mon *mon = to_mon(hw);
	...
	if (should_init_cpu_grp) {
		...
		list_add_rcu(&cpu_grp->node, &cpu_grp_list);
		if(cpu_grp->enable_polling)
			INIT_DEFERRABLE_WORK(&cpu_grp->work,
					     &memlat_monitor_work);
	}

	if (mon->miss_ev) {
		for_each_cpu(cpu, &mon->cpus) {
			unsigned int idx = to_mon_idx_safe(cpu, &mon->cpus);
	...
	if (should_init_cpu_grp && cpu_grp->enable_polling)
		queue_cpugrp_work(cpu_grp);
...
static void stop_hwmon(struct lealt_hwmon *hw)
{
	unsigned int cpu;
	struct memlat_mon *mon = to_mon(hw);
	struct memlat_cpu_grp *cpu_grp = mon->cpu_grp;
	...
	if (!cpu_grp->num_active_mons) {
		...
		list_del_rcu(&cpu_grp->node);
		synchronize_srcu(&lealt_srcu);
		free_common_evs(cpu_grp);
		smp_store_release(&cpu_grp->initialized, false);
	}
	mutex_unlock(&cpu_grp->mons_lock);
}
...
static void request_update_ms(struct lealt_hwmon *hw, unsigned int update_ms)
{
...
static int memlat_cpu_grp_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(dev->of_node, "enable_polling"))
		cpu_grp->enable_polling = true;
...
static int memlat_mon_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32_array(dev->of_node, "prepare",
		(u32 *)hw->prepare,
		(size_t)(ARRAY_SIZE(hw->prepare)))) {
		dev_err(dev, "Couldn't find prepare-minlock property.\n");
	 }

	if (of_property_read_u32(dev->of_node, "stability_th", &hw->stability_th))
		dev_err(dev, "Couldn't find stability_th property.\n");

	if (of_property_read_u32(dev->of_node, "base-minlock-ratio",
				 &hw->base_minlock_ratio))
		dev_err(dev, "Couldn't find base-minlock-ratio property.\n");

	if (of_property_read_u32(dev->of_node, "base-maxlock-ratio",
				 &hw->base_maxlock_ratio))
		dev_err(dev, "Couldn't find base-maxlock-ratio property.\n");

	if (of_property_read_u32(dev->of_node, "llc_on_th_cpu",
				 &hw->llc_on_th_cpu))
		dev_err(dev, "Couldn't find llc_on_th_cpu property.\n");
...
struct platform_driver arm_memlat_mon_driver = {
